 ...
 * store entries.
 * <p>
 * As specified in the {@link FieldMatrix} interface, matrix element indexing is
 * 0-based -- e.g., <code>getEntry(0, 0)</code> returns the element in the first
...
public class Array2DRowFieldMatrix<T extends FieldElement<T>> extends AbstractFieldMatrix<T> implements Serializable {
	...
	/**
	 * Create a new FieldMatrix<T> using the input array as the underlying data
	 * array.
	 * <p>
	 * The input array is copied, not referenced. This constructor has the same
	 * effect as calling
	 * {@link #Array2DRowFieldMatrix(FieldElement[][], boolean)} with the second
	 ...
	 */
	public Array2DRowFieldMatrix(final T[][] d) throws IllegalArgumentException, NullPointerException {
	...
	/**
	 * Create a new (column) FieldMatrix<T> using <code>v</code> as the data for
	 * the unique column of the <code>v.length x 1</code> matrix created.
	 * <p>
	 ...
	 */
	public Array2DRowFieldMatrix<T> add(final Array2DRowFieldMatrix<T> m) throws IllegalArgumentException {
	 ...
	 *             if m is not the same size as this
	 */
	public Array2DRowFieldMatrix<T> subtract(final Array2DRowFieldMatrix<T> m) throws IllegalArgumentException {
	 ...
	 */
	public Array2DRowFieldMatrix<T> multiply(final Array2DRowFieldMatrix<T> m) throws IllegalArgumentException {
		...
		if (data == null) {
			...
			for (int i = 0; i < data.length; ++i) {
				if (subMatrix[i].length != nCols) {
					throw MathRuntimeException.createIllegalArgumentException(
							"some rows have length {0} while others have length {1}", nCols, subMatrix[i].length);
				}
