...
/**
 * Simple implementation of the {@link VectorialConvergenceChecker} interface
 * using only point coordinates.
 * <p>
 * Convergence is considered to have been reached if either the relative
 * difference between each point coordinate are smaller than a threshold or if
 * either the absolute difference between the point coordinates are smaller than
 * another threshold.
 * </p>
 * 
...
public class SimpleVectorialPointChecker implements VectorialConvergenceChecker, Serializable {
	 ...
	 * set to a negative value. In order to perform only absolute checks, the
	 * relative tolerance must be set to a negative value.
	 * </p>
	 * 
