...
public class BinomialDistributionImpl extends AbstractIntegerDistribution
	...
	public void setNumberOfTrials(int trials) {
		if (trials < 0) {
			throw MathRuntimeException.createIllegalArgumentException("number of trials must be non-negative ({0})",
					trials);
		}
	...
	public double probability(int x) {
		...
		if (x < 0 || x > getNumberOfTrials()) {
			...
			ret = MathUtils.binomialCoefficientDouble(getNumberOfTrials(), x) * Math.pow(getProbabilityOfSuccess(), x)
					* Math.pow(1.0 - getProbabilityOfSuccess(), getNumberOfTrials() - x);
		}
		return ret;
	}
