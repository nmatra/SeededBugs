...
public class TDistributionImpl extends AbstractContinuousDistribution implements TDistribution, Serializable {
	...
	public void setDegreesOfFreedom(double degreesOfFreedom) {
		if (degreesOfFreedom <= 0.0) {
			throw MathRuntimeException.createIllegalArgumentException("degrees of freedom must be positive ({0})",
					degreesOfFreedom);
		}
	...
	public double cumulativeProbability(double x) throws MathException {
		...
		if (x == 0.0) {
			...
			double t = Beta.regularizedBeta(getDegreesOfFreedom() / (getDegreesOfFreedom() + (x * x)),
