 ...
 * first implement makeDistribution() to return a distribution instance to use
 * in tests. Then implement each of the test data generation methods below. In
 * each case, the test points and test values arrays returned represent parallel
 * arrays of inputs and expected values for the distribution returned by
 * makeDistribution(). Default implementations are provided for the
 * makeInverseXxx methods that just invert the mapping defined by the arrays
 * returned by the makeCumulativeXxx methods.
 * <p>
 ...
 * where possible, and the source of the reference data and/or validation should
 * be documented in the test cases. A framework for validating distribution data
 * against R is included in the /src/test/R source tree.
 * <p>
 * See {@link NormalDistributionTest} and {@link ChiSquareDistributionTest} for
 * examples.
 * 
...
public abstract class ContinuousDistributionAbstractTest extends TestCase {
	...
	/**
	 * Verifies that cumulative probability density calculations match expected
	 * values using current test instance data
	 */
	protected void verifyCumulativeProbabilities() throws Exception {
		for (int i = 0; i < cumulativeTestPoints.length; i++) {
			TestUtils.assertEquals("Incorrect cumulative probability value returned for " + cumulativeTestPoints[i],
					cumulativeTestValues[i], distribution.cumulativeProbability(cumulativeTestPoints[i]),
					getTolerance());
		}
	}
	 ...
	 * expected values using current test instance data
	 */
	protected void verifyInverseCumulativeProbabilities() throws Exception {
		for (int i = 0; i < inverseCumulativeTestPoints.length; i++) {
			TestUtils.assertEquals(
					"Incorrect inverse cumulative probability value returned for " + inverseCumulativeTestPoints[i],
					inverseCumulativeTestValues[i],
					distribution.inverseCumulativeProbability(inverseCumulativeTestPoints[i]), getTolerance());
		}
	}
	...
	/**
	 * Verifies that cumulative probability density calculations match expected
	 * values using default test instance data
	 */
	...
	/**
	 * Verifies that inverse cumulative probability density calculations match
	 * expected values using default test instance data
	 */
	...
	public void testConsistency() throws Exception {
		for (int i = 1; i < cumulativeTestPoints.length; i++) {
			...
			TestUtils.assertEquals(0d,
					distribution.cumulativeProbability(cumulativeTestPoints[i], cumulativeTestPoints[i]), tolerance);
			...
			TestUtils.assertEquals("Inconsistent cumulative probabilities for (" + lower + "," + upper + ")", diff,
					direct, tolerance);
		}
	}
